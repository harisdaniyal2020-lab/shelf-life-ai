...
const Index = () => {
  ...
  return <div className="min-h-screen bg-background/95">
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="flex flex-col items-center text-center space-y-12">
            ...
            <div className="animate-fade-in w-full max-w-3xl">
              <img src={heroImage} alt="Smart home inventory management system" className="w-full h-auto rounded-2xl shadow-2xl" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-24 px-4 sm:px-6 lg:px-8 bg-card/70 backdrop-blur-sm border-y border-border">
        <div className="container mx-auto">
          ...
          }].map((benefit, index) => <div key={index} className="flex flex-col items-center justify-center text-center gap-4 p-8 bg-background/80 rounded-lg shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 animate-fade-in">
      ...
      <section id="how-it-works" className="py-24 px-4 sm:px-6 lg:px-8 bg-background/95">
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-12 max-w-5xl mx-auto">
            <AnimatedFeatureStep number="1" icon={ScanBarcode} title="Scan & Add" description="Quickly scan grocery receipts, product barcodes (for food, toothpaste, cleaners, etc.), or input items manually." animationClass="group-hover:animate-scan" />
            <AnimatedFeatureStep number="2" icon={Sparkles} title="Smart Tracking" description="ShelfLife automatically categorizes items, estimates expiry dates for perishables, and tracks usage patterns for consumables." animationClass="group-hover:animate-twinkle" />
            <AnimatedFeatureStep number="3" icon={Bell} title="Alerts & Suggestions" description="Get notifications for expiring food, low stock on essentials, and smart recommendations for reordering or using up items." animationClass="group-hover:animate-ring" />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-fade-in">
              <img src={mariaImage} alt="Maria, a satisfied ShelfLife AI customer in her organized kitchen" className="w-full h-auto rounded-2xl shadow-xl" />
            </div>
      ...
      <section id="faq" className="py-24 px-4 sm:px-6 lg:px-8 bg-background/95">
        <div className="container mx-auto max-w-3xl">
          ...
          <Accordion type="single" collapsible className="space-y-4">
            <FAQItem value="item-1" question="Is scanning every item time-consuming?" answer="No, receipt scanning adds multiple items at once, and barcode scanning is instant. Most users spend < 5-10 minutes per shopping trip. You can also import previous online orders." />
            <FAQItem value="item-2" question="What about items without barcodes or expiry dates?" answer="Easy manual entry with smart suggestions for common items (e.g., 'toothpaste,' 'detergent') and usage tracking to predict when you'll run out." />
            <FAQItem value="item-3" question="Can my whole family use it?" answer="Yes, easily share your household inventory with family members, so everyone is on the same page for shopping and usage." />
          </Accordion>
      ...
      <section id="pricing" className="py-24 px-4 sm:px-6 lg:px-8 bg-card/70 backdrop-blur-sm border-y border-border">
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <ExpandablePricingCard title="Home Hub" price="$6.99" description="billed annually, save 15%" features={["Unlimited Items & Storage", "Advanced Recipe Integration", "Usage Tips for All Products", "Expiry & Low-Stock Alerts", "Receipt & Barcode Scanning", "Smart Shopping Lists"]} detailedFeatures={["Multi-location tracking (pantry, fridge, bathroom, garage)", "Custom categories and tags", "Recipe suggestions based on available ingredients", "Meal planning assistant", "Product usage tips and hacks", "Email and push notifications", "Mobile app access (iOS & Android)", "Export shopping lists to your phone"]} highlighted={true} onCTAClick={() => setIsContactFormOpen(true)} />
            <ExpandablePricingCard title="Family Sync" price="$10.99" description="up to 5 users" features={["Everything in Home Hub", "Up to 5 Family Members", "Shared Inventory Access", "Consolidated Shopping Lists", "Family Usage Analytics", "Priority Support"]} detailedFeatures={["Real-time sync across all family devices", "Individual user profiles with permissions", "Collaborative shopping lists", "Activity log to see who added/removed items", "Family consumption patterns and insights", "Shared recipe collections", "Priority email and chat support", "Early access to new features"]} onCTAClick={() => setIsContactFormOpen(true)} />
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="flex gap-8">
                <button onClick={() => openLegalModal("privacy")} className="text-sm text-muted-foreground hover:text-primary transition-all hover:scale-110 duration-300">
                  Privacy Policy
                </button>
                <button onClick={() => openLegalModal("terms")} className="text-sm text-muted-foreground hover:text-primary transition-all hover:scale-110 duration-300">
                  Terms of Service
                </button>
                <button onClick={() => setIsContactFormOpen(true)} className="text-sm text-muted-foreground hover:text-primary transition-all hover:scale-110 duration-300">
                  Contact Us
                </button>
              </div>
            </div>
